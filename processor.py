...
def update_schedule_fa(input_excel: str, output_csv: str, year: int):
    ...
    for ticker, df in xls.items():
        ...
        # Prefetch stock history once per ticker from 5 days before its earliest acquisition date for better backfill coverage
        ...
        for _, row in df.iterrows():
            ...
            if pd.isna(acq_date):
                raise ValueError(
                    f"Invalid acquisition date for {ticker}: {acquisition_raw}"
                )
            ...
            eff_qty_peak = _effective_quantity()
            ...
            eff_qty_close = _effective_quantity()
            closing_value = round(closing_price * eff_qty_close * rate_closing, 2)

            # Dividends (per-share payouts, scale by split-adjusted quantity and FX)
            ...
            if dividends_series is not None and not pd.Series(dividends_series).empty:
                ...
                for dt, div_per_share in div_in_period.items():
                    ...
                    # Aggregate by FY quarter
                    quarter = _fy_quarter_from_date(dt)
            ...
            total_dividends_inr = round(total_dividends_inr, 2)
            ...
            other_columns = {
                ...
                if k != "Quantity" and k not in base_details
            }
...
def _fy_quarter_from_date(dt) -> str:
